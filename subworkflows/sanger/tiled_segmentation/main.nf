...
process STARDIST {
    ...
    container "${ workflow.containerEngine == 'singularity' && !task.ext.singularity_pull_docker_container ?
        ...
        "quay.io/bioinfotongli/tiled_stardist:${container_version}"}"
    containerOptions = {
            workflow.containerEngine == "singularity" ? "--cleanenv --nv -B ${params.stardist_model_dir}:/cellpose_models":
            ( workflow.containerEngine == "docker" ? "--gpus all -v ${params.stardist_model_dir}:/cellpose_models": null )
    }
